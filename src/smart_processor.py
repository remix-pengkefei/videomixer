...
from .video_classifier import (
    classify_video_file, ClassificationResult, VideoCategory,
    print_classification_result
)
from .smart_effects import (
    get_smart_effect_config, describe_effect_config, CATEGORY_NAMES
)
...
def smart_process_video(
    ...
    Args:
        input_path: 输入视频路径
        output_path: 输出视频路径（默认在同目录生成 xxx_smart.mp4）
        verbose: 是否输出详细信息
    ...
    if output_path is None:
        ...
        output_path = str(input_p.parent / f"{input_p.stem}_smart{input_p.suffix}")
    ...
    try:
        ...
        classification = classify_video_file(input_path)
        result.category = classification.category
        result.category_name = CATEGORY_NAMES.get(classification.category, "未知")
        ...
        config = get_smart_effect_config(classification)

        if verbose:
            description = describe_effect_config(config, classification.category)
            print(description)
        ...
        width = features.width or 720
        height = features.height or 1280
        fps = int(features.fps) or 30
...
def batch_smart_process(
    ...
    Args:
        input_dir: 输入目录
        output_dir: 输出目录（默认在输入目录下创建 smart_output）
        extensions: 支持的视频扩展名
        verbose: 是否输出详细信息
    ...
    if output_dir is None:
        output_dir = os.path.join(input_dir, "smart_output")
    ...
    for i, video_path in enumerate(video_files, 1):
        ...
        output_path = os.path.join(
            output_dir,
            f"{video_path.stem}_smart{video_path.suffix}"
        )

        result = smart_process_video(
            str(video_path),
            output_path,
            verbose=verbose
        )
...
# 便捷函数
def process(input_path: str, output_path: Optional[str] = None) -> ProcessingResult:
    """
    智能处理单个视频（简化接口）
    """
    return smart_process_video(input_path, output_path, verbose=True)


def process_batch(input_dir: str, output_dir: Optional[str] = None) -> list:
    """
    批量智能处理（简化接口）
    """
    return batch_smart_process(input_dir, output_dir, verbose=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("VideoMixer 智能视频处理器")
        print("\n用法:")
        print("  处理单个视频: python -m src.smart_processor <视频路径> [输出路径]")
        print("  批量处理:     python -m src.smart_processor --batch <输入目录> [输出目录]")
        sys.exit(1)
    ...
    if sys.argv[1] == "--batch":
        ...
        batch_smart_process(input_dir, output_dir)
    else:
        input_path = sys.argv[1]
        output_path = sys.argv[2] if len(sys.argv) > 2 else None
        result = smart_process_video(input_path, output_path)
