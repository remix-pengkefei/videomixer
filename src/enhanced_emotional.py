...
def build_filter(w: int, h: int, sticker_files: list, sparkle_files: list,
                 video_index: int = 0, video_type: str = "emotional") -> tuple:
    """构建滤镜"""
    filters = []
    ...
    particle_str, particle_style = get_particle_filters(
        w, h, top_h, bottom_h, particle_colors, 50, video_index)
...
def process(input_path: str, output_path: str, video_index: int = 0) -> bool:
    ...
    print(f"\n{'=' * 60}")
    print("情感类视频超强混剪 (全效果池轮换版)")
    print(f"{'=' * 60}")
    print(f"输入: {Path(input_path).name}")
    print(f"时长: {duration:.1f}秒")

    assets_dir = Path(__file__).parent.parent / "assets"
    stickers = get_rotated_stickers(assets_dir, 20, "emotional", video_index)
    sparkles = get_sparkle_overlays(assets_dir, 5, "pink")

    w, h = 720, 1280
    video_filter, audio_filter, info = build_filter(
        w, h, stickers, sparkles, video_index, "emotional")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("用法: python -m src.enhanced_emotional <视频> [输出]")
        sys.exit(1)

    inp = sys.argv[1]
    out = sys.argv[2] if len(sys.argv) > 2 else str(Path(inp).parent / f"{Path(inp).stem}_情感版.mp4")
